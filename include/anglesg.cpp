...
AnglesGResult anglesg(
    ...
    Matrix colC(3,1);
    colC(1,1)=C_1; colC(2,1)=C_2; colC(3,1)=C_3;
    ...
    do{
        ...
        auto ELEM = elements(y);
        double p=ELEM.p;
        double mag2=r2.norm();
        double u_now=GM_Earth/(mag2*mag2*mag2);
        double rdot=Matrix::dot(r2,v2)/mag2;
        double udot=-3*GM_Earth*rdot/(mag2*mag2*mag2*mag2);
        double f1=1-0.5*u_now*tau1*tau1 - udot*tau1*tau1*tau1/6.0 - u_now*u_now*tau1*tau1*tau1*tau1/24.0 - udot*tau1*tau1*tau1*tau1*tau1/30.0;
        double g1=tau1 - u_now*tau1*tau1*tau1/6.0 - udot*tau1*tau1*tau1*tau1/12.0 - u_now*u_now*tau1*tau1*tau1*tau1*tau1/120.0 - udot*tau1*tau1*tau1*tau1*tau1*tau1/120.0;
        double f3=1-0.5*u_now*tau3*tau3 - udot*tau3*tau3*tau3/6.0 - u_now*u_now*tau3*tau3*tau3*tau3/24.0 - udot*tau3*tau3*tau3*tau3*tau3/30.0;
        double g3=tau3 - u_now*tau3*tau3*tau3/6.0 - udot*tau3*tau3*tau3*tau3/12.0 - u_now*u_now*tau3*tau3*tau3*tau3*tau3/120.0 - udot*tau3*tau3*tau3*tau3*tau3*tau3/120.0;
        C_1 = g3/(f1*g3 - f3*g1);
        C_2 = -1.0;
        C_3 = -g1/(f1*g3 - f3*g1);
        colC(1,1)=C_1; colC(2,1)=C_2; colC(3,1)=C_3;
